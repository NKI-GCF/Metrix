...
public class MetrixLogic {
  ...
  public boolean processMetrics(Path runDir, int st, DataStore ds) {
    ...
    if (state == Constants.STATE_INIT) {
      ...
      try {
        ...
        metrixLogger.log.log(Level.SEVERE, "Error parsing XML with SAX. {0}", SAX.toString());
      }
      catch (IOException Ex) {
        metrixLogger.log.log(Level.SEVERE, "IOException Error. {0}", Ex.toString());
      }
      catch (ParserConfigurationException PXE) {
        metrixLogger.log.log(Level.SEVERE, "Parser Configuration Exception. {0}", PXE.toString());
      }
      finally {
        xmd.closeAll();
      }
    ...
    if (summary.getParseError() >= 20 && summary.getCurrentCycle() > 20) {
      if ((em.getLastModifiedSourceDiff() >= Constants.ACTIVE_TIMEOUT) &&
        ...
        if (!summary.getPairedTurnCheck()) {    // Check if this is not a run waiting for turning.
          summary.setState(Constants.STATE_HANG);
          saveEntry(path);
          metrixLogger.log.log(Level.INFO, "Run {0} has failed to complete within the allotted time frame.", summary.getRunId());
          return false;
        }
      }
    }
    ...
    try {
      ...
      if (summary.getPairedTurnCheck()) {
        ...
        if (!summary.getHasNotifyTurned()) {
          summary.setState(Constants.STATE_TURN);
          metrixLogger.log.log(Level.INFO, "Flowcell of run: {0} has to be turned. Current cycle: {1}", new Object[]{path, currentCycle});
      ...
      metrixLogger.log.log(Level.INFO, "Finished processing: {0}", runDir.getFileName());
      success = true;
    }
  ...
  private void checkSummary(String path) {
    ...
    if (scrape || !summary.getRunDirectory().equals(path)) {
      try {
        ...
        metrixLogger.log.log(Level.SEVERE, "Error checking for summary by runId in database. {0}", SEx.toString());
      }
    }
  }
  ...
  public void finishRun(String path) {
    ...
    try {
      ...
      metrixLogger.log.log(Level.SEVERE, "Error setting up database connection. {0}", IE.toString());
      metrixLogger.log.log(Level.SEVERE, "Error stacktrace: {0}", IE);
    }
    metrixLogger.log.log(Level.INFO, "Run {0} has finished.", summary.getRunId());
    saveEntry(path);
    ...
    try {
      ...
      if (!_ds.checkSummaryByRunId(path)) {
        try {
          ...
          metrixLogger.log.log(Level.SEVERE, "Exception in write statement lastID = {0} Error: {1}", new Object[]{lastId, Ex.toString()});
        }
      }
      else {
        ...
        try {
          ...
          metrixLogger.log.log(Level.SEVERE, "Exception in update statement {0}", SEx.toString());
        }
      }
      _ds.closeAll();
      if (_ds != null) {
      ...
      metrixLogger.log.log(Level.SEVERE, "Run ID Checking error.{0}", Ex.toString());
    }
  }
  ...
  public boolean checkPaired(String path, DataStore ds) {
    ...
    if (summary.getPairedTurnCheck()) {
      ...
      summary.setState(Constants.STATE_TURN);
      metrixLogger.log.log(Level.INFO, "Flowcell of run: {0} has to be turned. Current cycle: {1}", new Object[]{summary.getRunId(), cCycle});
      summary.setHasNotifyTurned(true);
      check = true;
    }
    return check;
  ...
  public boolean checkTimeout(String file) {
    ...
    if (!lastModCheck.isDirectory()) {
      metrixLogger.log.log(Level.SEVERE, "Directory {0} does not exist.", file);
      return false;
    }
    ...
    if (difference > 86400000 && (summary.getState() == Constants.STATE_RUNNING)) { // If no updates for 24 hours. (86400000 milliseconds)
      metrixLogger.log.log(Level.INFO, "Run ({0}) has timed out. No data received for over 24 hours. Halting watch.", summary.getRunId());
  ...
  public void loadConfig() {
    try {
      ...
      metrixLogger.log.log(Level.SEVERE, "IOException when loading config: {0}", Ex.toString());
    }
  ...
  public boolean runPostProcessing(Summary sum) {
    ...
    if (execPP.equalsIgnoreCase("TRUE")) {
      ...
      metrixLogger.log.log(Level.INFO, "No postprocessing performed for: {0}", sum.getRunId());
      return false;
    }
  }
}
