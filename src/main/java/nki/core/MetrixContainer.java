...
public class MetrixContainer {
  ...
  public MetrixContainer(Summary summary, boolean remote) {
    ...
    if (sum != null) {
      ...
      timeCheck = (currEpoch - sum.getLastUpdatedEpoch()) > Constants.METRIC_UPDATE_TIME & !(this.sum.getHasFinished() || this.sum.getState() == Constants.STATE_FINISHED);
  ...
  /*
   * MetrixContainer to support a standalone approach where the whole run
   * directory is parsed and formatted according to a set specification.
   */
  public MetrixContainer(String runDir) {
    /*
     * Initialize the paths that point to the InterOp files required to be
     * processed.
     */
    initPaths(runDir);
  ...
  private void initSummary() {
    ...
    if ((!sum.hasClusterDensity() || !sum.hasClusterDensityPF() || !sum.hasPhasing() || !sum.hasPrephasing() || timeCheck || force) && !this.remote) {
      ...
      sum.setPhasingMap(tm.getPhasingMap()); // Get all values for summary and
                                             // populate
    ...
    if (((!sum.hasIntensityDistAvg() || !sum.hasIntensityDistCCAvg() || !sum.hasIntensityDistRaw()) || timeCheck || force) && !this.remote) {
      log.debug("Processing Corrected Intensity Metrics");
      if (!cim.getFileMissing()) {
    ...
    if ((!sum.hasIntensityDistRaw() || timeCheck || force) && !this.remote) {
    ...
    // Load IndexMetrics
    if ((!sum.hasSampleInfo() || force) && !this.remote) {
    ...
    // Load ErrorMetrics
    if ((!sum.hasErrorDist() || force) && !this.remote) {
      if (sum.getCurrentCycle() > 52) {
        log.debug("Processing Error Metrics");
    ...
    if ((update == true || force) && !this.remote) {
      try {
