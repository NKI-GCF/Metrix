...
public class MetrixErrorMetricsDecorator {
  ...
  public JSONObject toJSON() {
    ...
    /*
     * JSONArray cycles = new JSONArray(); for (int cycle :
     * eDist.getRunErrorDistributionByCycle().keySet()) { JSONObject lerr = new
     * JSONObject(); lerr.put("cycle", cycle); Map<Integer, List<Double>>
     * errorMap = eDist.getRunErrorDistributionByCycle().get(cycle); JSONArray
     * errors = new JSONArray(); for (int numErrors : errorMap.keySet()) {
     * JSONObject err = new JSONObject(); err.put("num", numErrors);
     * err.put("meanError",
     * Math.floor(ArrayUtils.mean(errorMap.get(numErrors)))); errors.add(err); }
     * lerr.put("errors", errors); cycles.add(lerr); } json.put("byCycle",
     * cycles);
     */
