  ...
  public JSONObject toJSON() {
    ...
    for (ListIterator<Summary> iter = sc.getSummaryCollection().listIterator(); iter.hasNext();) {
      ...
      if (this.expectedType.equals(Constants.COM_TYPE_SIMPLE)) {
        ...
        JSONObject tileMetrics = new MetrixTileMetricsDecorator(procSum.getClusterDensity(), procSum.getClusterDensityPF(), procSum.getPhasingMap(), procSum.getPrephasingMap(), procSum.getReads()).toJSON();
        ...
        JSONObject intensityMetrics = new MetrixIntensityMetricsDecorator(procSum.getIntensityDistAvg(), procSum.getIntensityDistCCAvg()).toJSON();
    ...
    try {
      ...
      for (ListIterator<Summary> iter = sc.getSummaryCollection().listIterator(); iter.hasNext();) {
        ...
        if (this.expectedType.equals(Constants.COM_TYPE_SIMPLE)) {
          ...
          // tile = new
          // MetrixTileMetricsDecorator(mc.getSummary()).toXML(sumXml, xmlDoc);
          sumXml.appendChild(tile);

          Element quality = xmlDoc.createElement("qualityMetrics");
          // quality = new
          // MetrixQualityMetricsDecorator(mc.getSummary().getQScoreDist()).toXML();
          sumXml.appendChild(quality);

          Element error = xmlDoc.createElement("errorMetrics");
          // error = new
          // MetrixErrorMetricsDecorator(mc.getSummary().getErrorDist()).toXML();
